...
The first macOS connection closes the verified official process, waits for its application services to settle, and relaunches the same LaunchServices identity without forcing a duplicate instance. Caller-owned Node and Electron process controls are removed from the launch environment. The runtime selects the primary `app://-/index.html` renderer and excludes auxiliary pages such as `avatar-overlay`. Injection begins only after the main process and its loopback CDP endpoint remain stable. A verified running loopback session can be adopted when local state is missing, and the session record is committed before renderer mutation so an injection retry does not require another restart. A failed debug launch is not reported as recovered until a normal Codex relaunch also passes a stability window.
